...
- Git
  - mainとdevを分けて運用
  - devに対して機能の実装をしたブランチをPR
  - コミットは極力意味で分けるようにし、コミットメッセージにprefixを付与
    - ex) Update, Fix, Change, Rename, Add...
- 簡潔性・可読性・安全性・保守性の高いコード
  - 一方向の依存を意識した
    - [アーキテクチャ図](https://github.com/tyPhoon-collab/yumemi-codecheck-repo-search/pull/56)
- Dart の言語機能を適切に使いこなせているか
  - [ブランチ構文](https://dart.dev/language/branches)などを使用
- テスト
  - riverpodを用いたDI
    - モックへの置換が簡単
...
- CI/CD
  ...
  - GitHub Actions
    - `flutter test`の自動化
    - Codecovとの連携
      - `bash calculate_coverage.sh`の自動化
...
- GitHub REST API 構築のために、[retrofit](https://pub.dev/packages/retrofit)と[freezed](https://pub.dev/packages/freezed)を使用することにした
  - 良くテストされているはずの既存のREST API Wrapperを使用することで、負担を軽減する
  - freezedを使用することにより、immutableかつ、便利なモデルクラスを構築する

- FlutterにはSearchDelegateというクラスがあり、showSearchを呼び出すことで検索が可能だが、挙動を柔軟にカスタマイズしたいかつ、showSearchを呼び出してTextFieldを表示する仕様のため、検索が主体のアプリでは適していないと考え、使用しない
- 検索バーのデザインはTextFieldをカスタマイズしてもよいが、SearchBarやSearchAnchorが完成されたデザインを提供しているので、こちらを使用する
- issueやforkなどのGitHubに関連するアイコンは[こちら](https://primer.style/foundations/icons/)から参照する
- アニメーションの追加
- ローカライズの実装
- テーマの実装、ダークモードの対応
- この時点で要件を満たすアプリは作成済み
...
- 全体的なリファクタリングを行った
  - ファイル分けなど
  - 適切にファイル分けすることで、AIによるリファクタリング、Widgetテストの作成が捗った
- テストを追加
...
- タイトルのデザインを修正
  - 太字に変更
  - ローカライズもしたが、日本語では少しダサいので、あえて英語を用いている
- 設定画面を追加
- エラーの修正やリファクタリング
...
- 改ページ機能の追加
  ...
  - [ここ](https://docs.github.com/ja/rest/using-the-rest-api/using-pagination-in-the-rest-api?apiVersion=2022-11-28)に詳しく書いてあるが、応答ヘッダーに次のページや最後のページの情報が含まれている。bodyのtotalCountが大きな値であっても、最大のページ数が決まっている
    - ただし、変更を意識してコーディングしていたので、サービスクラスのシグネチャを変えても、たった三箇所の修正で済んだ
...
- UI/UXの向上
  ...
  - 結果として、[MenuAnchor](https://api.flutter.dev/flutter/material/MenuAnchor-class.html)を採用した
    ...
    - Dropdownと比べて必要なスペースが少ない
      - 横画面対応の際に有利
      - SearchBarにIconButtonとして配置。右側に配置するボタンは２個まで許されている（[参考](https://m3.material.io/components/search/guidelines)）
...
- READMEを更新
- Codecovとの連携
  - actというツールをつかってGitHut Actionsをローカルでデバッグしていたが、actでは制限が多く、実態とは違い時間を取られてしまった
    - runs-onでmacOSが使えない
